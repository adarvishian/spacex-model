...
def run_formula_pass(workbook_path: Path) -> FormulaPassResult:
    ...
    result = FormulaPassResult(
        workbook_path=workbook_path, sheet_names=list(wb.sheetnames)
    )
...
def run_value_pass(workbook_path: Path) -> ValuePassResult:
    ...
    for row_idx in range(1, assumptions_ws.max_row + 1):
        ...
        for year, col_idx in year_cols.items():
            year_values[year] = _coerce_number(
                assumptions_ws.cell(row_idx, col_idx).value
            )
    ...
    for row_idx in range(1, ws.max_row + 1):
        ...
        if label.startswith("BB Q breakpoint"):
            result.bb_breakpoints.append(
                DemandCurveBreakpoint(
                    label=label, quantity_gbps=float(q_val), revenue=float(rev_val)
                )
            )
...
_ingest_cache: dict[tuple[str, float], IngestResult] = {}


def clear_ingest_cache() -> None:
    """Reset process-local ingest memo (for tests)."""
    _ingest_cache.clear()


def ingest_workbook(workbook_path: Path) -> IngestResult:
    """Run formula pass then value pass; memoized per resolved path + mtime."""
