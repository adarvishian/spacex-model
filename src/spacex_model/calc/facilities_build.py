...
def _straight_line_da(capex_lines: list[np.ndarray], lives: list[float]) -> np.ndarray:
...
def compute_facilities_build(inputs: FacilitiesBuildInputs) -> FacilitiesBuildResult:
    ...
    sat_base = assumption_scalar(a, cl.SAT_FACTORY_BASE_CAPACITY_2025_SATS_YR)
    sat_incr = assumption_scalar(a, cl.SAT_FACTORY_CAPACITY_SATS_YR_PER_INCREMENT)
    sat_cost = assumption_scalar(a, cl.SAT_FACTORY_COST_PER_CAPACITY_INCREMENT_MM)
    sat_exp = assumption_scalar(a, cl.SAT_FACILITY_CAPACITY_RATIO_CAPEX_EXPONENT)
    sat_life = assumption_scalar(a, cl.SAT_FACTORY_USEFUL_LIFE_YEARS)
    gs_per_yr = assumption_scalar(a, cl.GROUND_STATIONS_BUILT_PER_YEAR_PLACEHOLDER_FLAT)
    gs_cost = assumption_scalar(a, cl.GROUND_STATION_BUILD_CAPEX_PER_STATION_MM)
    gs_life = assumption_scalar(a, cl.GROUND_STATION_USEFUL_LIFE_YEARS)
    ...
    sat_mfg_da = _straight_line_da(
        [new_sat_factory_capex, ground_station_capex], [sat_life, gs_life]
    )

    # --- Bucket 2: Starship / Gigabay / engines / pads ---
    gigabay_base = assumption_scalar(a, cl.GIGABAY_BASE_CAPACITY_2025_SHIPS_YR)
    cap_ceiling = assumption_scalar(a, cl.STARSHIP_BUILD_CAPACITY_CEILING_SHIPS_YR)
    cap_ramp = assumption_scalar(
        a, cl.MAX_STARSHIP_BUILD_CAPACITY_ADDED_PER_YEAR_SHIPS_YR
    )
    op_year = int(assumption_scalar(a, cl.STARSHIP_OPERATIONAL_YEAR))
    ships_per_gigabay = assumption_scalar(
        a, cl.STARSHIP_FACILITY_CAPACITY_SHIPS_YR_PER_GIGABAY
    )
    gigabay_cost = assumption_scalar(
        a, cl.STARSHIP_FACILITY_COST_PER_GIGABAY_INCREMENT_MM
    )
    ship_fac_life = assumption_scalar(a, cl.STARSHIP_FACILITY_USEFUL_LIFE_YEARS)
    raptor_booster = assumption_scalar(a, cl.RAPTOR_ENGINES_PER_BOOSTER_SUPER_HEAVY)
    raptor_ship = assumption_scalar(a, cl.RAPTOR_ENGINES_PER_SHIP_2ND_STAGE)
    eng_base = assumption_scalar(a, cl.ENGINE_FACILITY_BASE_CAPACITY_2025_ENGINES_YR)
    eng_incr = assumption_scalar(
        a, cl.ENGINE_FACILITY_CAPACITY_ENGINES_YR_PER_INCREMENT
    )
    eng_cost = assumption_scalar(a, cl.ENGINE_FACILITY_COST_PER_CAPACITY_INCREMENT_MM)
    eng_life = assumption_scalar(a, cl.ENGINE_FACILITY_USEFUL_LIFE_YEARS)
    launches_per_pad = assumption_scalar(a, cl.STARSHIP_LAUNCHES_PER_PAD_PER_YEAR)
    pads_base = assumption_scalar(
        a, cl.LAUNCH_PADS_BASE_IN_SERVICE_2025_STARSHIP_CAPABLE
    )
    pad_cost = assumption_scalar(a, cl.LAUNCH_PAD_BUILD_UPGRADE_COST_MM_PER_PAD)
    pad_life = assumption_scalar(a, cl.LAUNCH_PAD_USEFUL_LIFE_YEARS)
    ...
    for t in range(HORIZON_YEARS):
        ...
        if eng_incr > 0:
            new_engine_capex[t] = (
                max(0.0, engine_demand[t] - prior_eng) / eng_incr * eng_cost
            )
    ...
    term_toggle = (
        ...
        else assumption_scalar(a, cl.TERMINAL_FACTORY_CAPEX_TOGGLE_1_ON_0_OFF)
    )
    terminal_capex = np.zeros(HORIZON_YEARS, dtype=np.float64)
    terminal_da = np.zeros(HORIZON_YEARS, dtype=np.float64)
    if term_toggle > 0:
        term_incr = assumption_scalar(
            a, cl.TERMINAL_FACTORY_CAPACITY_KITS_YR_PER_INCREMENT
        )
        term_cost = assumption_scalar(
            a, cl.TERMINAL_FACTORY_COST_PER_CAPACITY_INCREMENT_MM
        )
        term_life = assumption_scalar(a, cl.TERMINAL_FACTORY_USEFUL_LIFE_YEARS)
    ...
    hq_pct = assumption_scalar(a, cl.HQ_FACILITY_CAPEX_OF_REVENUE)
    hq_life = assumption_scalar(a, cl.HQ_FACILITY_USEFUL_LIFE_YEARS)
    ...
    wspm_per_phase = assumption_scalar(a, cl.TERAFAB_CAPACITY_PER_FAB_PHASE_WSPM_PHASE)
    fab_all_in = assumption_scalar(a, cl.TERAFAB_FAB_CAPEX_BLENDED_ALL_IN_WSPM)
    constr_window = int(assumption_scalar(a, cl.TERAFAB_FAB_CONSTRUCTION_WINDOW_YEARS))
    fab_life = assumption_scalar(a, cl.TERAFAB_FAB_USEFUL_LIFE_DEPRECIATION_YEARS)
    spacex_share = assumption_scalar(a, cl.TERAFAB_SPACEX_COST_CAPACITY_SHARE_FRAC)
    ...
    for t in range(HORIZON_YEARS):
        ...
        new_wspm[t] = (
            installed_wspm[t]
            if t == 0
            else max(0.0, installed_wspm[t] - installed_wspm[t - 1])
        )
...
def facilities_conservation_ok(result: FacilitiesBuildResult) -> bool:
    ...
    return bool(
        np.all(
            result.conservation_max_bucket.values <= CONSERVATION_RESIDUAL_TOLERANCE_MM
        )
    )
