...
def _per_sat_model_a_revenue_mm(assumptions: Assumptions, year_index: int) -> float:
    ...
    """
    compute_kw = assumption_scalar(assumptions, "Compute power per sat (kW)")
    coreweave = assumption_year_vector(
        assumptions,
        "CoreWeave baseline anchor ($B/GW_IT/yr, 2026) — year-row",
        default=12.0,
    )
    pue_uplift = assumption_scalar(assumptions, "Orbital PUE uplift vs terrestrial")
    util = assumption_scalar(assumptions, "ODC utilization factor")
    gw = compute_kw / 1e6
    baseline = (
...
def _per_sat_model_b_revenue_mm(assumptions: Assumptions, year_index: int) -> float:
    ...
    chip_tdp = assumption_year_vector(
        assumptions, "Chip TDP per chip (W) — year-row", default=700.0
    )
    chip_fp8 = assumption_year_vector(
        assumptions, "Chip FP8 performance per chip (TFLOPS) — year-row", default=1979.0
    )
    util = assumption_scalar(assumptions, "ODC utilization factor")
    ecr = assumption_scalar(assumptions, "Effective Compute Ratio (ECR)")
    mix = assumption_scalar(assumptions, "Workload mix — inference share")
...
def per_sat_combined_revenue_mm(assumptions: Assumptions, year_index: int) -> float:
    ...
    """
    pr_a = assumption_scalar(assumptions, "Credence on Model A (Pr(A))")
...
def per_sat_bandwidth_cost_mm(
    ...
    if starlink_capacity is None:
        return 0.0
    bb_share = assumption_scalar(assumptions, "BB-share of ODC bandwidth claim")
    gbps_per_gwh = assumption_scalar(
        assumptions, "Gbps per GWh/yr of ODC compute energy"
    )
    compute_kw = assumption_scalar(assumptions, "Compute power per sat (kW)")
...
def per_sat_net_marginal_revenue_mm(
    ...
    ground = assumption_scalar(assumptions, "ODC ground ops % of revenue")
    insurance = assumption_scalar(assumptions, "ODC insurance % of revenue")
    other = assumption_scalar(assumptions, "ODC other COGS % of revenue")
...
def odc_bandwidth_claim(inputs: OdcInputs) -> tuple[YearVector, YearVector]:
    ...
    bb_share = assumption_scalar(inputs.assumptions, "BB-share of ODC bandwidth claim")
    gbps = assumption_scalar(
        inputs.assumptions, "Gbps per GWh/yr of ODC compute energy"
    )
    compute_kw = assumption_scalar(inputs.assumptions, "Compute power per sat (kW)")
...
def compute_fcf(inputs: OdcInputs | None = None) -> YearVector:
    ...
    if inputs is None:
        return YearVector.zeros()
    return YearVector(
        compute_gross_profit(inputs).values - compute_capex(inputs).values
    )
...
def per_sat_blended_irr(inputs: OdcInputs) -> float:
    ...
    if a.lookup("V3 BB sat unit cost ($mm/sat)") is not None:
        cost = assumption_scalar(a, "V3 BB sat unit cost ($mm/sat)")
    n = int(assumption_scalar(a, "ODC fleet design life (years)"))
    rev = np.array(
        [
