...
def _unit_cost_mm_vec(
    assumptions: Assumptions, unit_cost_label: str, *, cost_kg: float, mass_kg: float
) -> np.ndarray:
...
def _v2_vehicle_cash_demand(
    ...
    *,
    ...
    facility = assumption_year_vector(
        assumptions, facility_per_sat_label, default=0.0
    ).values
...
def _v3_vehicle_cash_kg_demand(
    ...
    *,
    ...
    launches = assumption_year_vector(
        assumptions, stub_trajectory_label, default=0.0
    ).values
