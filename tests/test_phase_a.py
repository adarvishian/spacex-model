...
def test_ingest_assumptions() -> None:
    ...
    assert assumptions.starting_cash_eoy_2024 == 11_385


@pytest.mark.skipif(not LEGACY_WORKBOOK.exists(), reason="V2.16 workbook not present")
def test_base_case_pipeline() -> None:
    result = run_base_case(LEGACY_WORKBOOK, write_outputs=False)
...
def test_settings_default_workbook() -> None:
    ...
    assert "V4.113" in settings.workbook_path.name
