...
export type ScenarioRunArtifact = {
  ...
  /** Full API payload when generated by current precompute; legacy artifacts may be partial. */
...
export function hasClientRunPayload(deterministic: DeterministicRun | undefined): boolean {
  if (!deterministic?.valuation || deterministic.modules == null) return false;
  return Number.isFinite(deterministic.valuation.group_ev_2025_b);
}

/** Loaded precache artifact can drive the UI (sha match not required). */
export function canUsePrecacheRunArtifact(
  scenario: string,
  overrides: Record<string, number>,
): boolean {
  if (!isPrecachedScenario(scenario)) return false;
  if (Object.keys(overrides).length > 0) return false;
  return runCache.has(scenario);
}
...
export function canHydrateRunFromArtifact(
  ...
  if (!canUsePrecacheRunArtifact(scenario, overrides)) return false;
