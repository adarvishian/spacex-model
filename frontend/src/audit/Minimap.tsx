...
export function Minimap({
  ...
  return (
    <aside className="minimap" aria-label="Model minimap">
      <h2>Model Map</h2>
          ...
          return (
            <div key={group.id}>
              ...
              {nodes.map((node) => {
                ...
                const feeds =
                  !isActive &&
                  upstream.some((u) => u.label.toLowerCase().includes(node.label.toLowerCase()));
                const depends =
                  !isActive &&
                  downstream.some((d) => d.label.toLowerCase().includes(node.label.toLowerCase()));
                return (
                  <button
                    key={node.slug}
                    type="button"
